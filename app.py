...
def analyze_and_update(image, context: str) -> tuple[dict | None, str]:
    if not active_model:
        return None, "Clé API invalide"
    try:
        model = genai.GenerativeModel(active_model)
        prompt = f"""
Tu es un expert en marchés publics français. Contexte: {context}.
Analyse ce document et retourne UNIQUEMENT un objet JSON valide (sans markdown, sans texte autour) :
{{
    "match": 85,
    "rse": "Fort",
    "delay": "6 mois",
        ...
        if p.get("analysis_done"):
            ...
            if dc1_available:
                ...
                st.markdown(
                    ...
                    "⚠️ <strong>Modèle DC1 officiel non disponible</strong> — "
                ...
                with st.spinner("Génération du DC1..."):
                    try:
                        ...
                        if mode == "officiel":
                            ...
                            st.info("ℹ️ PDF de substitution généré. Chargez le modèle officiel pour obtenir le Cerfa interactif.")
                    except Exception as e:
            ...
            with st.expander("⚙️ Charger / Remplacer le modèle DC1 officiel"):
                ...
                if model_upload:
                    ...
                    if nb_fields > 0:
                        ...
                        st.warning(
                            "⚠️ Ce PDF ne semble pas contenir de champs interactifs AcroForm. "
